...
class CNN(object):
    def __init__(self):
        ...
        model.add(layers.Conv2D(
            64, (3,3), strides=(1,1),activation='relu',  input_shape=(IMG_SHAPE, IMG_SHAPE, 1), name='conv1'))
        model.add(layers.MaxPooling2D((2, 2), strides=2, name='pooling1'))
        # 第2层卷积，卷积核大小为3*3，64个
        model.add(layers.Conv2D(128, (3, 3), strides=(1,1), activation='relu',name='conv2'))
        model.add(layers.MaxPooling2D((2, 2), strides=2,name='pooling2'))
        # 第3层卷积，卷积核大小为3*3，64个
        model.add(layers.Conv2D(256, (3, 3), strides=(1,1), activation='relu',name='conv3'))
        # model.add(tf.keras.layers.Dropout(0.5))

        model.add(layers.Flatten(name='f1'))
        model.add(layers.Dense(128, activation='relu',name='d1'))
...
class DataSource(object):
    def __init__(self):
        data_path = os.path.abspath(os.path.dirname(
            __file__)) + '/../Dataset/normalinandroid.npz'
        ...
        train_images  = train_images.reshape((OUTSIZE*Samplesize,IMG_SHAPE,IMG_SHAPE,1))
        test_images = test_images.reshape((OUTSIZE*TestS,IMG_SHAPE,IMG_SHAPE,1))
...
class Train:
    ...
    def train(self):
        ...
        self.cnn.model.fit(self.data.train_images, self.data.train_labels,batch_size=12,
                           epochs=10, callbacks=[save_model_cb])


        test_loss, test_acc = self.cnn.model.evaluate(self.data.test_images, self.data.test_labels)
        print("准确率: %.4f，共测试了%d张图片 " % (test_acc, len(self.data.test_labels)))
        self.cnn.model.save('./model/normalinandroid_cnn.h5')
